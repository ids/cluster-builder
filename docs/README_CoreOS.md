...
> It is somewhat ironic that we use the __bare metal__ install for the VMware environment.  These days bare metal has come to mean anything buy cloud environments, which is silly.  It would have been more accurate to refer to this as the PXE/iPXE install process.  It works very well for VMware.
...
The GUI Installer does a good job of illustrating the process, so once you use it the first time, things will make more sense.  

1. Define the configuration
2. Validate it.  
3. __Apply__ it (via Terraform/Matchbox).  
4. Boot up the nodes, which will then request their configurations using their MAC addresses as the key to their ID.

Only after the application reaches the point where the configuration has been submitted to Matchbox are the Node VMs powered on.  It is an asynchronous provisioning in the spirit of a reconcilliation loop and it works very well.

> If they try to contact Matchbox before their MAC addresses are registered in a cluster configuration, they will be rejected, and a message "Operating system not found" will appear.
...
It isn't fully automated yet... only partially.  But given that the entire stack is self updating and managed by the CoreOS team, soon to be RedHat, the need to re-deploy a cluster often seems unlikely.  Once the layout is right, it will likely have a long shelf life (unlike some whale based distros).
...
That said, the PXE approach produces a pure, undoctored CoreOS that is quite nice.
...
* manual-masters.csv (controllers are also called masters)
* manual-workers.csv
